...
namespace PJC.Models
{
    public class StoreContext
    {
        ...
        public List<muasach> GetMuaSach()
        {
            List<muasach> list = new List<muasach>();

            using (MySqlConnection conn = GetConnection())
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from muasach ", conn);

                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                            list.Add(new muasach()
                            {
                                MaDonHang = int.Parse(reader["MaSoMuaSach"].ToString()),
                                HoTen = reader["HoTen"].ToString(),
                                SoDienThoai = reader["SoDienThoa"].ToString(),
                                DiaChi = reader["DiaChi"].ToString(),
                                MaSach = reader["maSach"].ToString(),
                                TinhTranThanhToan = int.Parse(reader["ThanhToan"].ToString()),
                                TinhTranDon = int.Parse(reader["TinhTranDonHang"].ToString()),
                                NguoiBan = reader["User"].ToString()
                            });
        ...
        public int XoaDonHangMuaSach(int id)
        {
            using (MySqlConnection conn = GetConnection())
            {
                conn.Open();
                var str = "Delete from muasach where MaSoMuaSach=@ms";
                MySqlCommand cmd = new MySqlCommand(str, conn);
                cmd.Parameters.AddWithValue("ms", id);
                return (cmd.ExecuteNonQuery());
            }
        }
        public int BanDonHangMuaSach(int id,string Masach,string user)
        {
            int soluongsach = 0;
            using (MySqlConnection conn3 = GetConnection())
            {
                ...
                var str2 = "update sach set SoLuong=@soluong where MaSach=@masach";
                MySqlCommand cmd2 = new MySqlCommand(str2, conn3);
                cmd2.Parameters.AddWithValue("soluong", soluongsach - 1);
                cmd2.Parameters.AddWithValue("masach", Masach);
                cmd2.ExecuteNonQuery();


                var str = "update muasach set ThanhToan=@thanhtoan,TinhTranDonHang=@tinhtrang, User=@user where MaSoMuaSach=@ms";
                MySqlCommand cmd = new MySqlCommand(str, conn3);
                cmd.Parameters.AddWithValue("ms", id);
                cmd.Parameters.AddWithValue("thanhtoan", 1);
                cmd.Parameters.AddWithValue("tinhtrang", 1);
                cmd.Parameters.AddWithValue("user", user);
                return (cmd.ExecuteNonQuery());
            }
        }
        public int CreateMuaSach(muasach a)
        {
            using (MySqlConnection conn = GetConnection())
            {
                conn.Open();
                var str = "insert into muasach values(@Maso, @HoTen,@SoDienThoa,@DiaCHi,@ThanhThoan,@Masch,@TInhTranDonHang,@User)";
                MySqlCommand cmd = new MySqlCommand(str, conn);
                cmd.Parameters.AddWithValue("Maso", null);
                cmd.Parameters.AddWithValue("HoTen", a.HoTen);
                cmd.Parameters.AddWithValue("SoDienThoa", a.SoDienThoai);
                cmd.Parameters.AddWithValue("DiaCHi", a.DiaChi);
                cmd.Parameters.AddWithValue("ThanhThoan", a.TinhTranThanhToan);
                cmd.Parameters.AddWithValue("Masch", a.MaSach);
                cmd.Parameters.AddWithValue("TInhTranDonHang", a.TinhTranDon);
                cmd.Parameters.AddWithValue("User", a.NguoiBan);
                return (cmd.ExecuteNonQuery());
